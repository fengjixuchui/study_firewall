...
int capitalisepath(const char *string, char **result) {
	...
	if( !( *result = (char *)malloc( strlen( string ) + 1 ) ) ) {
        ...
        printf("capitalisepath malloc failed with error %d: %s\n", errno, strerror( errno ));
		return -1;
	}
	...
	for( max = strlen( string ) - 1 ; max > 0 ; --max ) {
		...
		printf("capitalisepath read char %c\n", c );

		if( c == '/' )
			break;
	}

	printf("capitalisepath last separator is at %d\n", max );
	...
	for( unsigned int n = 0 ; n < max ; n++ ) {
		...
		if( 0 == n ) {
			...
			printf("capitalisepath using char %c\n", (*result)[ n ] );
		}

		printf("capitalisepath lastchar is %c\n", lastchar );
	}
	for( ; max < strlen( string ) ; max++ ) {
		unsigned char c = string[ max ];
		(*result)[ max ] = c;

		printf("capitalisepath appending char %c\n", c );
	}
	(*result)[ strlen( string ) ] = '\0';

	printf("capitalisepath string is %s\n", *result );

	len = strlen( *result );

	printf("capitalisepath returning %lu\n", len );
...
int hooked_open(const char *path, int flags, ...) {
	...
	if( 1 == m ) {
        ...
        printf("->  Intercepted three-arg open('%s', %d, %ju)", pathname, flags, CONVERT_TO_MAX(mode_t, mode) );
	} else {
		verbose_fprintf( stderr, "->  Intercepted two-arg open('%s', %d)", pathname, flags );
        printf("->  Intercepted two-arg open('%s', %d)", pathname, flags );
	}
